...
async function saveToSupabase(mois, kpisJson) {
  ...
  const rows = Object.entries(kpisJson)
    ...
    .map(([name, val]) => {
      ...
      return {
        mois,
        parc: name.trim(), // ✅ TRIM : supprime les espaces Skello parasites
  ...
  console.log(`📤 Upsert ${rows.length} parcs pour ${mois}:`, rows.map(r => r.parc));

  // Upsert dans Supabase (met à jour si (mois, parc) existe déjà)
